...
`lib.typ`, `interpret.typ` and `exif.wasm` must sit next to each other; nothing
...
make test       # Rust tests, then the Typst test suites
```
...
| `interpret.typ` | the interpreting layer, used internally by `read-exif` |
